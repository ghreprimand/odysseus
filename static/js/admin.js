...
function _diagBrief(data) {
  ...
  if (errors) return `${errors} check${errors === 1 ? '' : 's'} need attention.`;
  if (warnings) return `${warnings} check${warnings === 1 ? '' : 's'} degraded; ${ok} healthy.`;
  if (skipped) return `${ok} checks healthy; ${skipped} optional item${skipped === 1 ? '' : 's'} not configured.`;
